...
function getSoilData(lat, lon) {
  fetch(`https://api.openepi.io/soil/query?lat=${lat}&lon=${lon}`)
    .then(res => res.json())
...
function displaySoilData(data) {
    document.getElementById("soil-moisture").textContent = data.moisture ?? "N/A";
    document.getElementById("soil-ph").textContent = data.ph ?? "N/A";
    document.getElementById("soil-type").textContent = data.type ?? "N/A";
}
